...
##### 1단계: 기본 차이점 점수 (30점 만점)
```
차이도 = 100 - 평균 알고리즘 점수

if (차이도 >= 80): 점수 += 30  // 매우 다른 답변 (유사도 20% 이하)
else if (차이도 >= 60): 점수 += 20  // 상당히 다른 답변 (유사도 40% 이하)
else if (차이도 >= 40): 점수 += 10  // 약간 다른 답변 (유사도 60% 이하)
else: 점수 += 0  // 유사한 답변 (유사도 60% 이상)
```
...
- "성인", "자유", "개방" 등 포함 시 점수 부여

**욕설 표현 카테고리:**
- 욕설 표현 관련 핵심 정보 포함 여부 평가
- 다양한 욕설 표현 중 2개 이상 포함 시 20점

#### 로그 변환 및 정규화
```
정규화된 점수 = log(원본 점수 + 1) / log(최대 가능 점수 + 1)
```
- 0-1 범위로 정규화
- 낮은 원점수 → 높은 정규화 점수
- 지수적 스케일링으로 극단값 완화
