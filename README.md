...
## 📋 목차
- [프로젝트 개요](#프로젝트-개요)
- [🤖 SFASS Red-Team Harness (신규·핵심)](#-sfass-red-team-harness)
- [주요 기능](#주요-기능)
- [평가 시스템 상세](#평가-시스템-상세)
- [기술 스택](#기술-스택)
- [설치 및 실행](#설치-및-실행)
...
```
llm-spear-shield/
├── sfass-redteam/        # 🤖 AI 에이전트 레드팀 플랫폼 "Banya Readteam Harness" (신규·핵심)
│   ├── src/              # 공격자·판정·타깃 어댑터·오케스트레이터·시나리오·기록 저장소
│   ├── dashboard/        # 공격·벤치 / 취약점 분석 / 리포트 대시보드 (단일 HTML)
│   ├── scripts/          # 로그인 캡처(Playwright) 등
│   ├── feature.md        # 제품 설명서
│   └── README.md         # 사용 가이드
...
## 🤖 SFASS Red-Team Harness
### AI 에이전트를 위한 올인원 레드팀 플랫폼 (Banya Readteam Harness)

[`sfass-redteam/`](sfass-redteam/) 은 본 저장소의 **신규 핵심 모듈**로, LLM·AI 에이전트가 실제 운영 환경에서 얼마나 안전한지 검증하는 **자동화 레드팀 플랫폼**입니다. 공인 AI 안전 대회 **SFASS 2026 Judgement Day**(주최: 인공지능안전연구소 · 주관: AIM Intelligence)에서 실물 휴머노이드(Unitree G1) 에이전트 공략 **first-solve**로 실전 검증되었습니다.

**핵심 기능**
- ⚔️ **다차원 공격 엔진** — 27종+ 최신 연구 기반 공격 전략 + **5개 도메인 시나리오**(피지컬AI/로봇 · 전자상거래 · 금융거래 · 업무자동화 · 성적안전 가드레일, 35+개)
- 🧬 **전략 생성기** — 잘 안 뚫리는 대상엔 사용 가능한 여러 AI가 아이디어를 내고 **Claude가 종합해 새 공격 전략을 자동 설계**, 쓸수록 전략 라이브러리에 누적
- 🌐 **즉시 연결** — 대상 **URL만 넣으면 AI가 요청/응답 스키마를 추론해 자동 배선**(+ 로그인 캡처 · 쿠키 연동 · 배선 검증)
- 📈 **실시간 공격 벤치 & 대시보드** — 시도별 페이로드·응답·판정 근거 카드 + 전략별 성공률 실시간 집계
- 🔬 **취약점 분석 & 경영진용 리포트** — 모델별 취약점 비교·집중 분석, **복합(연쇄) 위험도 예측**, **HTML/PDF 보고서**
- 🧠 **다중 모델 지원** — NVIDIA · Google Gemini · Anthropic Claude · OpenAI 호환을 **공격자/방어자/판정** 역할로 자유 배치, 실물 없이 **방어 에이전트 시뮬레이션**도 가능

**실증 성과**: 8개 모델 취약점 실측 — GLM 5.2 **완전 방어(0%)**, Gemma4·Qwen3.5 낮음(2–5%), 오픈 모델군 높음(17–55%). **간접 프롬프트 인젝션(업무자동화)**이 공통 최약점으로 확인.

> 📄 자세한 내용: **[제품 설명서](sfass-redteam/feature.md)** · **[사용 가이드](sfass-redteam/README.md)**
> ⚠️ **인가된 보안 테스트 전용** — 공인 대회, 본인이 소유·운영하거나 계약상 테스트 권한이 있는 시스템에 한해 사용하세요.
