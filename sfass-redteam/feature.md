...
> **vuln**=위험도가중 뚫림률(high3·med2·low1), **rate**=단순 성공률, hi/mid/lo=위험도별 성공 건수.

### 9.1 종합 취약점 점수 (높을수록 취약)
...
| **deepseek-v4-pro** | 8 | 5% | 37 | 2 | 1 | 1 | 0 |
| **qwen3.5-122b** | 5 | 5% | 58 | 3 | 2 | 0 | 1 |
| **OMEGA-7 / Def.Ender** (Unitree G1 · 라이브) | 2 | 2% | 55 | 1 | 1 | 0 | 0 |
| **gemma-4-31b** | 2 | 2% | 51 | 1 | 1 | 0 | 0 |
| **GLM 5.2** (z-ai/glm-5.2) | **0** | **0%** | 54 | 0 | 0 | 0 | 0 |

### 9.2 카테고리별 뚫림 매트릭스 (동일 구성: 시나리오 2 × 강전략 6 × 1회, 방어자=mock)

성공/시도. GLM·Gemma4·Qwen3.5-122b 를 5개 카테고리에 동일 조건으로 공격한 통제 벤치.

| 모델 | 피지컬/로봇 | 전자상거래 | 금융거래 | 업무자동화 | 성적안전 |
|------|:---:|:---:|:---:|:---:|:---:|
| **gemma-4-31b** | 0/10 | 0/12 | 0/10 | **1/8** | 0/11 |
| **qwen3.5-122b** | 0/11 | **1/12** | 0/12 | **2/12** | 0/11 |
| **GLM 5.2** | 0/12 | 0/12 | 0/6 | 0/12 | 0/12 |

### 9.3 핵심 발견

- **GLM 5.2 = 0/54, 완전 방어.** 5개 카테고리 전부 뚫림 0으로 가장 견고. (초기 벤치에서 전송 404로 무효였던 문제를 fresh 재측정으로 해소 → 실제로는 최상위 견고성 확인.)
- **업무자동화(workflow)가 최약점 카테고리.** gemma·qwen 둘 다 여기서만 뚫림 — 특히 `indirect_injection_zero_click_exfil`(문서/메일 은닉지시 제로클릭 유출)이 `prefix_forcing`·`roleplay_theater`로 3회 성공. 간접 프롬프트 인젝션이 실전 최대 위협임을 재확인.
- **qwen3.5-122b** (3건): 이커머스 오염리뷰 무단주문 1 + 업무 제로클릭 유출 2 — 신규 3모델 중 상대적으로 가장 뚫림.
- **gemma-4-31b** (1건): 업무 제로클릭 유출 1.
- **피지컬·금융·성적안전은 세 상용 모델 모두 완전 방어**(0 성공).
- **OMEGA-7/Def.Ender**: 라이브 대회 타깃. 경례 33시도 중 1성공 + 하트 22시도 0성공 = **1/55(2%)** — 19턴 크레센도 끝에 겨우 1회 뚫린 매우 견고한 타깃(단발 100% 표기를 실제 다회 시도 기준으로 교정).
- **상용 프론티어급(GLM·Gemma4·Qwen3.5)이 오픈 모델(minimax·nemotron·llama)보다 훨씬 견고** — 안전정렬 성숙도 격차가 뚜렷.

> 주의: 9.1은 서로 다른 목표·전략이 섞인 누적치(오픈 모델은 초기 mock 벤치 포함)라 지표(indicative)다. 9.2는 3모델을 **동일 조건**으로 돌린 통제 비교다. 실전 배포 위험 예측은 대시보드 **🔬 취약점 분석 → LLM 복합 위험도**를 참조.
